...
def test_populate_attachments_on_all_documents(migrator):
    ...
    old_state = migrator.apply_initial_migration(("core", "0019_alter_user_language_default_to_null"))
    OldDocument = old_state.apps.get_model("core", "Document")

    old_doc_without_attachments = OldDocument.objects.create(title="Doc without attachments", depth=1, path="0000002")
    old_doc_with_attachments = OldDocument.objects.create(title="Doc with attachments", depth=1, path="0000001")
    ...
    ydoc = pycrdt.Doc()
    fragment = pycrdt.XmlFragment([pycrdt.XmlElement("img", {"src": f"http://localhost/media/{image_key:s}"})])
    ...
    new_doc_without_attachments = NewDocument.objects.get(pk=old_doc_without_attachments.pk)
