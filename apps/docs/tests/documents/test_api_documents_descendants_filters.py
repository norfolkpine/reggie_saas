...
from apps.docs.api.filters import remove_accents
...
def test_api_documents_descendants_filter_unknown_field():
    ...
    response = client.get(f"/docs/api/v1/documents/{document.id!s}/descendants/?unknown=true")
...
def test_api_documents_descendants_filter_title(query, nb_results):
    ...
    response = client.get(f"/docs/api/v1/documents/{document.id!s}/descendants/?title={query:s}")
