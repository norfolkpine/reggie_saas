...
def test_api_documents_create_for_owner_missing_token():
    ...
    response = APIClient().post("/docs/api/v1/documents/create-for-owner/", data, format="json")
...
def test_api_documents_create_for_owner_invalid_token():
    ...
    response = APIClient().post(
        "/docs/api/v1/documents/create-for-owner/",
        data,
        format="json",
...
def test_api_documents_create_for_owner_existing_user_email_no_sub_no_fallback(
    ...
    assert response.status_code == 400
    assert response.json() == {"email": ["We couldn't find a user with this sub but the email is already associated with a registered user."]}
