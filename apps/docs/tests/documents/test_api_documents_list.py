...
def test_api_documents_list_anonymous(reach, role):
    ...
    response = APIClient().get("/docs/api/v1/documents/")
...
def test_api_documents_list_pagination(
    ...
    assert content["next"] == "http://testserver/docs/api/v1/documents/?page=2"
    ...
    assert content["previous"] == "http://testserver/docs/api/v1/documents/"
...
def test_api_documents_list_pagination_force_page_size():
    ...
    # Force page size
    response = client.get(
        "/docs/api/v1/documents/?page_size=2",
    )
    ...
    assert content["count"] == 3
    assert content["next"] == "http://testserver/docs/api/v1/documents/?page=2&page_size=2"
    assert content["previous"] is None
