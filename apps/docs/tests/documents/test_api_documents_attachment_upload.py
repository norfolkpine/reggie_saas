...
def test_api_documents_attachment_upload_anonymous_success():
    ...
    file_head = default_storage.connection.meta.client.head_object(Bucket=default_storage.bucket_name, Key=key)
...
def test_api_documents_attachment_upload_fix_extension(name, content, extension, content_type):
    """
    ...
    document.refresh_from_db()
    assert document.attachments == [f"{document.id!s}/attachments/{file_id!s}.{extension:s}"]
