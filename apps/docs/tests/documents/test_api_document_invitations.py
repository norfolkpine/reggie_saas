...
def test_api_document_invitations_list_anonymous_user():
    ...
    response = APIClient().get(f"/docs/api/v1/documents/{invitation.document.id!s}/invitations/")
...
def test_api_document_invitations_list_authenticated_privileged(role, via, mock_user_teams, django_assert_num_queries):
    ...
    with django_assert_num_queries(3):
        response = client.get(
            f"/docs/api/v1/documents/{document.id!s}/invitations/",
        )
...
def test_api_document_invitations_list_authenticated_unprivileged(role, via, mock_user_teams, django_assert_num_queries):
    """
...
def test_api_document_invitations_retrieve_anonymous_user():
    ...
    response = APIClient().get(
        f"/docs/api/v1/documents/{invitation.document.id!s}/invitations/{invitation.id!s}/",
    )
...
def test_api_document_invitations_create_email_from_senders_language():
    ...
    assert "Docs, votre nouvel outil incontournable pour organiser, partager et collaborer sur vos documents en équipe." in email_content
...
def test_api_document_invitations_update_authenticated_privileged_any_field_except_role(role, via, mock_user_teams):
    ...
    url = f"/docs/api/v1/documents/{invitation.document.id!s}/invitations/{invitation.id!s}/"
...
def test_api_document_invitations_delete_authenticated_outsider():
    ...
    response = client.delete(
        f"/docs/api/v1/documents/{document.id!s}/invitations/{invitation.id!s}/",
    )
    assert response.status_code == 403
