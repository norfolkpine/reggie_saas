...
def test_api_document_versions_list_anonymous(role, reach):
    ...
    response = APIClient().get(f"/docs/api/v1/documents/{document.id!s}/versions/")
...
def test_api_document_versions_list_authenticated_unrelated(reach):
    ...
    response = client.get(
        f"/docs/api/v1/documents/{document.id!s}/versions/",
    )
    assert response.status_code == 403
...
def test_api_document_versions_list_authenticated_related_pagination(via, mock_user_teams):
    ...
    # - set page size
    response = client.get(
        f"/docs/api/v1/documents/{document.id!s}/versions/?page_size=2",
    )
    ...
    # - get page 2
    response = client.get(
        f"/docs/api/v1/documents/{document.id!s}/versions/?page_size=2&version_id={marker:s}",
    )
...
def test_api_document_versions_list_exceeds_max_page_size():
    ...
    response = client.get(f"/docs/api/v1/documents/{document.id!s}/versions/?page_size=51")
...
def test_api_document_versions_retrieve_anonymous(reach):
    ...
    url = f"/docs/api/v1/documents/{document.id!s}/versions/{version_id:s}/"
...
def test_api_document_versions_retrieve_authenticated_unrelated(reach):
    ...
    response = client.get(
        f"/docs/api/v1/documents/{document.id!s}/versions/{version_id:s}/",
    )
    assert response.status_code == 403
    assert response.json() == {"detail": "You do not have permission to perform this action."}
...
def test_api_document_versions_update_authenticated_unrelated():
    ...
    response = client.put(
        f"/docs/api/v1/documents/{access.document_id!s}/versions/{version_id:s}/",
        {"foo": "bar"},
        format="json",
    )
    assert response.status_code == 405
...
def test_api_document_versions_delete_anonymous(reach):
    ...
    response = APIClient().delete(
        f"/docs/api/v1/documents/{access.document_id!s}/versions/{access.id!s}/",
    )
