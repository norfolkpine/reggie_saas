...
def test_api_documents_duplicate_forbidden():
    ...
    response = client.post(f"/docs/api/v1/documents/{document.id!s}/duplicate/")
...
def test_api_documents_duplicate_anonymous():
    ...
    response = APIClient().post(f"/docs/api/v1/documents/{document.id!s}/duplicate/")
...
def test_api_documents_duplicate_success(index):
    ...
    response = client.post(f"/docs/api/v1/documents/{document.id}/duplicate/")
    ...
    response = client.get("/docs/api/v1/documents/media-auth/", HTTP_X_ORIGINAL_URL=image_refs[0][1])
    ...
    for _, url in image_refs[1:]:
        response = client.get("/docs/api/v1/documents/media-auth/", HTTP_X_ORIGINAL_URL=url)
        assert response.status_code == 403
...
def test_api_documents_duplicate_with_accesses():
    ...
    response = client.post(
        f"/docs/api/v1/documents/{document.id!s}/duplicate/",
        {"with_accesses": True},
        format="json",
    )
