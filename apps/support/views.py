...
def api_hijack_user(request):
    ...
    if not request.user.is_superuser:
        return Response({"error": "Only superusers can impersonate users"}, status=status.HTTP_403_FORBIDDEN)
...
def api_stop_hijack(request):
    ...
    if not original_user_id:
        return Response({"error": "Not currently impersonating any user"}, status=status.HTTP_400_BAD_REQUEST)

    try:
        original_user = CustomUser.objects.get(id=original_user_id, is_active=True)
    except CustomUser.DoesNotExist:
        return Response({"error": "Original user not found"}, status=status.HTTP_404_NOT_FOUND)
...
def api_impersonation_status(request):
    ...
    return Response({"is_impersonating": False, "impersonated_user": None, "original_user": None})
