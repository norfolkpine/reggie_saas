...
def get_team_api_url_templates() -> dict[str, str]:
    ...
    invitation_url_templates = {api_url: reverse(api_url, args=[TEAM_PLACEHOLDER, INVITATION_PLACEHOLDER]) for api_url in invite_api_url_names}
    invitation_url_templates["teams:accept_invitation"] = reverse("teams:accept_invitation", args=[INVITATION_PLACEHOLDER])
    ...
    membership_url_templates = {api_url: reverse(api_url, args=[TEAM_PLACEHOLDER, tmp_integer_url_key]) for api_url in membership_api_url_names}
    # convert from numbers to the actual key
    membership_url_templates = {url_name: url.replace(str(tmp_integer_url_key), MEMBERSHIP_PLACEHOLDER) for url_name, url in membership_url_templates.items()}
