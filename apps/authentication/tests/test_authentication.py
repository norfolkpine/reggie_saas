...
def test_login_view_json():
    ...
    response = client.post(
        url, {'email': 'valid@example.com', 'password': 'validpass123'}, format='json'
    )
...
def test_login_with_invalid_email():
    ...
    url = reverse('authentication:rest_login')
    response = client.post(
        url, {'email': 'wrong@example.com', 'password': 'somepass123'}, format='json'
    )
...
def test_login_with_invalid_password():
    ...
    url = reverse('authentication:rest_login')
    response = client.post(
        url, {'email': 'user@example.com', 'password': 'wrongpass'}, format='json'
    )
