...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            type=str,
            required=True,
            help='Name of the collection/folder'
        )
        parser.add_argument(
            '--description',
            type=str,
            default='',
            help='Description of the collection/folder'
        )
        parser.add_argument(
            '--type',
            type=str,
        ...
        parser.add_argument(
            '--parent',
            type=int,
            help='ID of the parent collection (leave empty for root)'
        )
        parser.add_argument(
            '--jurisdiction',
            type=str,
            help='Jurisdiction (e.g., "Australia", "NSW")'
        )
        parser.add_argument(
            '--regulation-number',
            type=str,
            help='Regulation number (e.g., "2001", "No. 123")'
        )
        parser.add_argument(
            '--sort-order',
            type=int,
            default=0,
            help='Sort order within parent (default: 0)'
        )
    ...
    def handle(self, *args, **options):
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"✅ Successfully created collection: {collection.name}"
                )
            )
