...
class RBACService:
    ...
    def get_user_accessible_filters(user) -> dict[str, Any]:
        ...
        try:
            ...
            kb_permissions = KnowledgeBasePermission.objects.filter(team__in=user_teams).select_related(
                "knowledge_base"
            )

            kb_ids = [
                perm.knowledge_base.knowledgebase_id for perm in kb_permissions if perm.knowledge_base.knowledgebase_id
            ]
    ...
    def get_user_knowledge_bases(user) -> list[KnowledgeBase]:
        ...
        kb_permissions = KnowledgeBasePermission.objects.filter(team__in=user_teams).select_related("knowledge_base")
    ...
    def can_user_access_knowledge_base(user, knowledge_base_id: str) -> bool:
        ...
        try:
            ...
            user_teams = Team.objects.filter(members=user)
            return KnowledgeBasePermission.objects.filter(knowledge_base=kb, team__in=user_teams).exists()
    ...
    @staticmethod
    def get_permission_level(user, knowledge_base_id: str) -> str | None:
        """
        ...
        try:
            ...
            user_teams = Team.objects.filter(members=user)
            permission = (
                KnowledgeBasePermission.objects.filter(knowledge_base=kb, team__in=user_teams).order_by("-role").first()
    ...
    def validate_ingestion_metadata(user, metadata: dict[str, Any]) -> dict[str, str]:
        ...
        if metadata.get("team_id"):
            if not Team.objects.filter(id=metadata["team_id"], members=user).exists():
        ...
        if metadata.get("project_id"):
            try:
                ...
                user_teams = Team.objects.filter(members=user)
                if not TeamProject.objects.filter(id=metadata["project_id"], team__in=user_teams).exists():
