...
class ProjectInstruction(BaseModel):
    name = models.CharField(max_length=255, help_text="Name of this instruction template")
    content = models.TextField(help_text="The instruction/prompt content for AI processing")
    description = models.TextField(blank=True, null=True, help_text="Description of what this instruction does")
...
class Project(BaseModel):
    ...
    vault_vector_table = models.CharField(
        max_length=255, blank=True, null=True, help_text="Vector table name for Vault file embeddings"
    )
...
class VaultFile(models.Model):
    ...
    embedding_status = models.CharField(
        max_length=20, choices=INGESTION_STATUS_CHOICES, default="not_started", help_text="Current embedding status"
    )
...
class VaultFileInsight(models.Model):
    ...
    key_points = models.JSONField(default=list, blank=True, help_text="List of key points extracted from the file")
    extracted_entities = models.JSONField(
        default=dict, blank=True, help_text="Named entities extracted (people, places, dates, etc.)"
    )
    tags = models.JSONField(default=list, blank=True, help_text="AI-generated tags for categorization")
    file_type_category = models.CharField(
        max_length=100, blank=True, help_text="AI-determined file category (contract, report, etc.)"
    )
    ...
    ai_model_used = models.CharField(max_length=100, blank=True, help_text="AI model used for generating insights")
    ...
    processed_at = models.DateTimeField(null=True, blank=True, help_text="When the insights were generated")
...
class AiConversation(models.Model):
    ...
    context_files = models.JSONField(
        default=list, blank=True, help_text="List of file IDs used as context for this conversation"
    )
    tokens_used = models.IntegerField(default=0, help_text="Number of tokens used for this conversation")
    ai_model_used = models.CharField(max_length=100, blank=True, help_text="AI model used for this conversation")
...
class AiProcessingQueue(models.Model):
    ...
    vault_file = models.ForeignKey(
        VaultFile, on_delete=models.CASCADE, related_name="ai_processing_queue", help_text="File to process"
    )
    ...
    processed_at = models.DateTimeField(null=True, blank=True, help_text="When the task was processed")
