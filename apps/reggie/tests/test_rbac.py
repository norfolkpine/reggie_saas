...
class RBACServiceTest(TestCase):
    ...
    def setUp(self):
        ...
        self.kb1 = KnowledgeBase.objects.create(name="KB 1", knowledgebase_id="kb1-test", uploaded_by=self.user1)
        self.kb2 = KnowledgeBase.objects.create(name="KB 2", knowledgebase_id="kb2-test", uploaded_by=self.user2)

        # Create permissions
        KnowledgeBasePermission.objects.create(
            knowledge_base=self.kb1, team=self.team1, role=KnowledgeBasePermission.ROLE_VIEWER
        )
    ...
    def test_validate_ingestion_metadata_valid_team(self):
        ...
        metadata = {"team_id": str(self.team1.id), "knowledgebase_id": "kb1-test"}
...
class RBACFilteredRetrieverTest(TestCase):
    ...
    def test_rbac_filtered_retriever_initialization(self):
        ...
        with patch("apps.reggie.services.rbac_service.RBACService.get_user_accessible_filters") as mock_filters:
    ...
    def test_node_matches_filters_or_condition(self):
        ...
        filters = {"$or": [{"user_uuid": "user123"}, {"team_id": "team456"}]}
        retriever = RBACFilteredRetriever(self.base_retriever, self.user, filters=filters)
...
class RBACIntegrationTest(TestCase):
    ...
    def setUp(self):
        ...
        # Create users
        self.admin_user = User.objects.create_user(email="admin@test.com", username="admin", password="testpass123")
        self.regular_user = User.objects.create_user(email="user@test.com", username="user", password="testpass123")
        ...
        self.kb = KnowledgeBase.objects.create(
            name="Test KB", knowledgebase_id="test-kb-integration", uploaded_by=self.admin_user
        )

        # Create team permission
        KnowledgeBasePermission.objects.create(
            knowledge_base=self.kb, team=self.team, role=KnowledgeBasePermission.ROLE_EDITOR
        )
