...
from apps.reggie.models import File as ReggieFile
...
class GoogleDriveSync:
    ...
    def __init__(self, user: CustomUser, team: Team | None = None):
        self.user = user
        self.team = team
    ...
    def get_google_drive_files(self, folder_id: str | None = None, page_token: str | None = None) -> dict:
    ...
    def download_file(self, file_id: str, filename: str) -> ContentFile | None:
    ...
    def get_or_create_collection(
        self, name: str, parent: Collection | None = None, collection_type: str = "folder"
    ) -> Collection:
    ...
    def should_sync_file(self, mime_type: str, size: int | None = None) -> bool:
    ...
    def sync_file(self, file_data: dict, collection: Collection | None = None) -> bool:
        ...
        existing_file = ReggieFile.objects.filter(title=name, collection=collection, uploaded_by=self.user).first()
    ...
    def sync_folder_structure(
        ...
        parent_collection: Collection | None = None,
        depth: int = 0,
        max_depth: int = 10,
    ) -> None:
        ...
        for folder_data in folders:
            ...
            collection = self.get_or_create_collection(
                name=folder_name, parent=parent_collection, collection_type="folder"
            )

            # Recursively sync subfolder
            self.sync_folder_structure(
                folder_id=folder_id, parent_collection=collection, depth=depth + 1, max_depth=max_depth
            )
    ...
    def sync_all(self, root_folder_id: str | None = None, max_depth: int = 10) -> dict:
        ...
        try:
            ...
            self.sync_folder_structure(folder_id=root_folder_id, parent_collection=None, depth=0, max_depth=max_depth)
    ...
    request={
        "application/json": {
            ...
            "properties": {
                ...
                "team_id": {"type": "integer", "description": "Optional: Team ID to associate synced files with"},
            },
        }
    },
    responses={
...
def start_google_drive_sync(request):
    ...
    try:
        ...
        if max_depth < 1 or max_depth > 20:
            return Response({"error": "max_depth must be between 1 and 20"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if team_id:
            try:
                ...
                if not team.members.filter(id=request.user.id).exists():
                    return Response({"error": "You don't have access to this team"}, status=status.HTTP_403_FORBIDDEN)
            except Team.DoesNotExist:
                return Response({"error": "Team not found"}, status=status.HTTP_404_NOT_FOUND)

        # Start sync process
        sync_service = GoogleDriveSync(user=request.user, team=team)
        sync_stats = sync_service.sync_all(root_folder_id=root_folder_id, max_depth=max_depth)

        return Response({"message": "Google Drive sync completed successfully", "sync_stats": sync_stats})
...
def preview_google_drive_structure(request):
    ...
    try:
        ...
        if max_depth < 1 or max_depth > 10:
            return Response({"error": "max_depth must be between 1 and 10"}, status=status.HTTP_400_BAD_REQUEST)

        # Authenticate
        sync_service = GoogleDriveSync(user=request.user)
        if not sync_service.authenticate():
            return Response({"error": "Google Drive not connected"}, status=status.HTTP_401_UNAUTHORIZED)

        # Get folder structure
        def explore_folder(folder_id: str | None, depth: int = 0) -> dict:
            if depth > max_depth:
            ...
            for file in regular_files:
                children.append(
                    {"name": file["name"], "type": "file", "mime_type": file["mimeType"], "size": file.get("size")}
                )

            return {"name": "Root" if folder_id is None else "Folder", "type": "folder", "children": children}
        ...
        return Response({"folder_structure": structure, "file_count": file_count, "folder_count": folder_count})
...
def get_sync_status(request):
    ...
    try:
        ...
        synced_files = ReggieFile.objects.filter(
            uploaded_by=request.user, description__icontains="Synced from Google Drive"
        ).count()

        synced_collections = Collection.objects.filter(description__icontains="Synced from Google Drive").count()
