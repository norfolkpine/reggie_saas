...
def handle_slash_command(req: SocketModeRequest):
    ...
    try:
        ...
        requests.post(
            response_url, json={"text": "⚠️ Sorry, something went wrong while processing your translation request."}
        )
...
def handle_events_api(req: SocketModeRequest):
    ...
    try:
        client.web_client.reactions_add(name="eyes", channel=channel, timestamp=event["ts"])
    except Exception as e:
    ...
    try:
        if event_type == "app_mention":
            ...
            client.web_client.chat_postMessage(channel=channel, text=final_text, thread_ts=thread_ts)

        elif event_type == "message" and channel_type == "im":
            print(f"📩 DM from <@{user}>: {text}")
            response: RunResponse = agent.run(text)
            client.web_client.chat_postMessage(channel=channel, text=response.content.strip(), thread_ts=thread_ts)
