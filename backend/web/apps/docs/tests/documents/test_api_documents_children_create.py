...
from apps.docs.models import Document, LinkReachChoices, LinkRoleChoices
...
def test_api_documents_children_create_authenticated_title_null():
    ...
    response = client.post(f"/docs/api/v1/documents/{parent.id!s}/children/", {}, format="json")
...
def test_api_documents_children_create_force_id_success():
    ...
    response = client.post(
        f"/docs/api/v1/documents/{access.document.id!s}/children/",
        {
            "id": str(forced_id),
            "title": "my document",
        },
...
def test_api_documents_create_document_children_race_condition():
    ...
    def create_document():
        return client.post(
            f"/docs/api/v1/documents/{document.id}/children/",
            {
                "title": "my child",
            },
        )
