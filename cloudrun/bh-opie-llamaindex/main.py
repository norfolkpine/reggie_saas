...
from strategies.docling_splitter import DoclingSplitter # ADD THIS
...
async def process_single_file_stream(payload: FileIngestRequest, idempotency_key: str):
    ...
    try:
        ...
        if isinstance(text_splitter, DoclingSplitter):
            # Docling operates on a file path, so we need to download the file first.
            file_content = download_gcs_file(file_path)
            with tempfile.NamedTemporaryFile(delete=False, suffix=os.path.basename(file_path)) as tmp_file:
                tmp_file.write(file_content)
                tmp_file_path = tmp_file.name

            # Create a single Document that points to the local file path.
            # The splitter will use the metadata to find the file.
            doc_with_path = Document(metadata={"file_path": tmp_file_path})

            # The DoclingSplitter will handle both reading and parsing.
            nodes = text_splitter.get_nodes_from_documents([doc_with_path])

            # Clean up the temporary file
            os.unlink(tmp_file_path)

            # Apply the base metadata to the nodes generated by Docling.
            for node in nodes:
                node.metadata.update(base_metadata)
            all_chunks = nodes

        else:
            # === STANDARD TEXT-BASED CHUNKING (EXISTING LOGIC) ===
